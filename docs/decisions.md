...
- 迁移只能添加列，不能删除或修改现有列
- 依赖增量列的索引必须在对应 `ensure_column()` 之后创建，不能与基础表索引混合批次
...
## ADR-011: 分层测试策略驱动 AI 反馈闭环

### 状态
已采纳 | 2026-05-05

### 背景
项目已经进入 MVP 收敛期，主要风险从“缺功能”转向“AI 修改代码后引入回归、Mock 与真实 Tauri 契约漂移、任务生命周期边界被破坏”。需要一套轻量但可执行的测试文档体系，让 AI 生成代码后能够快速验证并根据失败结果迭代。

### 决策

采用分层测试策略：
- `docs/testing/strategy.md` 定义测试分层、验证命令和 AI 测试规则。
- `docs/testing/test-cases/` 记录功能测试用例与自动化状态。
- `docs/testing/acceptance-criteria/` 记录可直接映射到任务的验收标准。
- `docs/testing/mocks-and-fixtures.md` 约束浏览器 Mock、测试数据和临时数据规则。
- `docs/testing/regression-checklist.md` 作为提交和发布前的最小回归入口。

当前门禁以 `npm run build`、`cargo test` 和必要的手动 smoke 为主；Playwright 和前端单元测试先作为计划能力，不作为当前硬门禁。

### 理由
- **反馈明确**：AI 可以根据测试失败输出修正代码，而不是依赖人工描述。
- **防漂移**：Mock、API 契约、任务生命周期和文档可以互相校验。
- **轻量可执行**：先使用已有命令，避免在测试框架未落地前制造虚假门禁。
- **渐进增强**：测试用例先文档化，后续可逐步自动化。

### 约束
- 新增行为变更任务必须在 `tasks.md` 中包含验收标准和关联测试用例。
- 修复 Bug 时必须先记录复现用例或复现步骤。
- 不得把尚未配置的命令（如 `npm test`、`npm run test:e2e`）写成当前必跑门禁。
...
| 日期 | 版本 | 变更 |
|------|------|------|
| 2026-05-05 | v1.1 | 新增 ADR-011：分层测试策略驱动 AI 反馈闭环 |
