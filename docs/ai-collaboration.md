...
| 修改类型 | 必须读取的文档 |
|----------|---------------|
| 判断当前状态 / 下一步 | `tasks.md` → `roadmap.md` |
| 判断测试策略 / 验收方式 | `testing/strategy.md` → `test-plan.md` |
| 修改 UI 组件 | `design-doc.md` §3, §8 |
...
- 每次修改代码后，必须说明需要运行的最小验证命令。
- 修改业务逻辑、数据模型、API 契约或复杂 UI 交互时，必须新增或更新对应测试；如果暂时无法自动化，必须更新 `docs/testing/test-cases/` 或 `docs/testing/acceptance-criteria/`。
- 修复 Bug 时，先记录能复现 Bug 的测试用例或复现步骤，再修复。
- 不删除、不跳过现有测试，除非用户明确要求；如果必须调整测试，说明原因和风险。
- 测试名称应使用清晰业务语义，避免只描述实现细节。
- 浏览器 Mock 与 Tauri 真实命令必须共享同一契约；修改 `mock-invoke.ts` 时检查 `docs/testing/mocks-and-fixtures.md`。
- 最终回复必须列出已运行验证；未运行时说明原因和替代检查。
...
必读：
  ...
  3. docs/ai-collaboration.md  ← 本文档，确认协作红线
  4. docs/tasks.md             ← 当前实现状态和短期任务
  5. docs/roadmap.md           ← 当前阶段和优先级
  6. docs/testing/strategy.md  ← 测试策略和验证入口
  7. docs/design-doc.md        ← 系统设计，单一事实来源

按需读取：
  - api-contract.md            ← 修改命令时
  - data-model.md             ← 修改数据库时
  - design-doc.md §3, §8      ← 修改 UI 时
  - workflows.md              ← 修改流程时
  - test-plan.md              ← 需要完整测试清单时
  - testing/test-cases/       ← 需要关联测试用例时
  - testing/acceptance-criteria/ ← 需要验收标准时
  - changelog.md              ← 需要了解近期变化时
```
